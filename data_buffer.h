...
typedef struct _SPECIAL_DATA_HEADER
{
    union{
        int set[2];
        struct{
    ...
    // total 4KB/4B = 1k = 1024,1024 - entry of set = 1024 - 2 = 1022
    int resereve[1022];
}SPECIAL_DATA_HEADER, *P_SPECIAL_DATA_HEADER;


typedef struct _PARTIAL_DATA_ENTRY
{
    unsigned int transmit_data_address;
    unsigned int receive_data_address;
    unsigned int verify_data_address;
}PARTIAL_DATA_ENTRY, *P_PARTIAL_DATA_ENTRY;

typedef struct _PARTIAL_DATA_MAP
{
    PARTIAL_DATA_ENTRY partial_dataBuf[1];
} PARTIAL_DATA_MAP, *P_PARTIAL_DATA_MAP;
