...
extern int nvme_complete_flag;
extern int *testBufferPtr;
extern FILENAME_BUFFER *nmcMappingFilenameBufPtr;
P_ROW_ADDR_DEPENDENCY_TABLE rowAddrDependencyTablePtr;

bool verify_img_flag = false;
extern NMC_MAPPING_TABLE *nmcMappingTableBufPtr;
...
void ReqTransNvmeToSlice(unsigned int cmdSlotTag, unsigned int startLba, unsigned int nlb, unsigned int cmdCode)
{
    ...
    pr_debug("requestedNvmeBlock: %d",requestedNvmeBlock);
    pr_debug("transCounter: %d",transCounter);
    pr_debug("tempLsa: %d",tempLsa);
    pr_debug("loop: %d",loop);
    ...
    case IO_NVM_NMC_INFERENCE_READ:
        pr_debug("get partial result in ReqTransNvmeToSlice");
        // If there is no data in the firmware that can be transferred, just transfer 4KB of data.
        if(dataPartialResult->partial_dataBuf[0].transmit_data_address == dataPartialResult->partial_dataBuf[0].receive_data_address){
            ...
            reqCode = REQ_CODE_NMC_SPECIAL_HEADER;
            // dataPartialResult->partial_dataBuf[0].receive_data_address += 0x00005000; 
        }
        // if(dataPartialResult->partial_dataBuf[0].transmit_data_address == dataPartialResult->partial_dataBuf[0].receive_data_address){
        //     loop = 0;
        //     requestedNvmeBlock = 1;
        ...
        // }
            ...
            reqCode = REQ_CODE_NMC_INFERENCE_READ;
        }
    ...
    case IO_NVM_NMC_VERIFY_IMG:
        reqCode            = REQ_CODE_VERIFY_IMG;
        break;
    case IO_NVM_WRITE_BUFFER:
        pr_info("IO_NVM_WRITE_BUFFER in ReqTransNvmeToSlice");
        reqCode = REQ_CODE_WRITE_BUFFER;
        break;
    case IO_NVM_GET_MAPPING_TABLE:
        pr_info("IO_NVM_GET_MAPPING_TABLE in ReqTransNvmeToSlice");
        reqCode   = REQ_CODE_GET_MAPPING_TABLE;
        startLba  = LSA_NONE & (~(NVME_BLOCKS_PER_SLICE - 1)); 
        break;
    default:
    ...
    pr_debug("Request info:");
    pr_debug("reqCode :%d",reqCode);
    pr_debug("nvmeDmaInfo.startIndex :%d",nvmeDmaStartIndex);
    pr_debug("nvmeDmaInfo.nvmeBlockOffset :%d",nvmeBlockOffset);
    pr_debug("numOfNvmeBlock :%d",tempNumOfNvmeBlock);
...
void ReqTransSliceToLowLevel()
{
    ...
    while (sliceReqQ.headReq != REQ_SLOT_TAG_NONE)
    {
         ...
         *
         * If the data buffer not exists, we the newly created data must allocate a data buffer entry by calling
         * `AllocateDataBuf()` and initialize buffer.
         */

        /*
           Confirm whether the command is INFERENCE_READ ,if yes it will not be allocated buffer
        */

       /*
        * Distinguish whether data buffer needs to be allocated 
        * or whether this request has additional SSD DRAM receiving location       
       */
        if (REQ_ENTRY(reqSlotTag)->reqCode == REQ_CODE_NMC_INFERENCE_READ || REQ_ENTRY(reqSlotTag)->reqCode == REQ_CODE_WRITE_BUFFER){
            if(REQ_ENTRY(reqSlotTag)->reqCode == REQ_CODE_WRITE_BUFFER){
                pr_info("REQ_CODE_WRITE_BUFFER in ReqTransSliceToLowLevel");
                REQ_ENTRY(reqSlotTag)->dataBufInfo.addr = *testBufferPtr;
                *testBufferPtr =  *testBufferPtr + REQ_ENTRY(reqSlotTag)->nvmeDmaInfo.numOfNvmeBlock*4096;
            }
            else{
                pr_debug("inference read in allocate buffer");
                partial_result_address = dataPartialResult->partial_dataBuf[0].transmit_data_address;
                REQ_ENTRY(reqSlotTag)->dataBufInfo.addr = partial_result_address;
                pr_debug("inference read DRAM address info:0x%x",REQ_ENTRY(reqSlotTag)->dataBufInfo.addr);
                partial_result_address = partial_result_address + REQ_ENTRY(reqSlotTag)->nvmeDmaInfo.numOfNvmeBlock*4096;
                dataPartialResult->partial_dataBuf[0].transmit_data_address = partial_result_address;
                pr_debug("next inference read DRAM address info:0x%x",partial_result_address);
            }
        }
        else if(REQ_ENTRY(reqSlotTag)->reqCode == REQ_CODE_GET_MAPPING_TABLE)
        {
            partial_result_address = dataPartialResult->partial_dataBuf[0].verify_data_address;
            REQ_ENTRY(reqSlotTag)->dataBufInfo.addr = partial_result_address;
            pr_info("get_mapping_table buffer address:0x%x",REQ_ENTRY(reqSlotTag)->dataBufInfo.addr);
        }
        else{  
            ...
            else
            {
                ...
                */
                ...
                pr_debug("bufEntry info:0x%p",dataBufEntry);
            }
        }
        REQ_ENTRY(reqSlotTag)->reqOpt.dataBufFormat = REQ_OPT_DATA_BUF_ENTRY;
        ...
        case REQ_CODE_WRITE:
            ...
            else if (REQ_CODE_IS(reqSlotTag, REQ_CODE_VERIFY_IMG))
            {   
                ASSERT(nmcRegisterInferenceReq(reqSlotTag), "verify register failed...");
                verify_img_flag = true;
                BUF_ENTRY(dataBufEntry)->dirty = DATA_BUF_CLEAN; /* don't flush this */
            }
            else
        ...
        case REQ_CODE_NMC_INFERENCE_READ:
        case REQ_CODE_NMC_SPECIAL_HEADER:
        case REQ_CODE_GET_MAPPING_TABLE:
            pr_debug("inference read set DMA request");
            REQ_ENTRY(reqSlotTag)->reqCode     = REQ_CODE_TxDMA;
            REQ_ENTRY(reqSlotTag)->reqOpt.dataBufFormat = REQ_OPT_DATA_BUF_ADDR;
            break;
        case REQ_CODE_OCSSD_PHY_READ:
        case REQ_CODE_READ:
        ...
        case REQ_CODE_WRITE_BUFFER:
            pr_info("test input buffer request");
            REQ_ENTRY(reqSlotTag)->reqCode     = REQ_CODE_RxDMA;
            REQ_ENTRY(reqSlotTag)->reqOpt.dataBufFormat = REQ_OPT_DATA_BUF_ADDR;
            break;
        default:
        ...
        /*
         * Distinguish whether reqOpt.dataBufFormat is "REQ_OPT_DATA_BUF_ADDR", 
         * which means this transmission is not based on "data buffer entry"
         * Requires additional settings
        */ 
        if (REQ_ENTRY(reqSlotTag)->reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ADDR ){
            pr_info("request dataBufFormat is REQ_OPT_DATA_BUF_ADDR");
            if (REQ_ENTRY(reqSlotTag)->reqCode == REQ_CODE_RxDMA)
            {
                pr_info("REQ_CODE_WRITE_BUFFER set done");
            }
            REQ_ENTRY(reqSlotTag)->reqType              = REQ_TYPE_NVME_DMA;
            pr_info("\t reqCode = 0x%x", REQ_ENTRY(reqSlotTag)->reqCode);
            pr_info("\t dataBufaddress = 0x%x", REQ_ENTRY(reqSlotTag)->dataBufInfo.addr);
        }
        // Because the current communication between the host and firmware involves specifying addresses 
        // through data buffer entries when we need to use a memory address to indicate the transferred data
        // we should ensure that "reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ADDR" is not updated in the "UpdateDataBufEntryInfoBlockingReq." 
        // This is to prevent the occurrence of an issue where two requests using memory addresses may block each other.
        else{
            REQ_ENTRY(reqSlotTag)->reqType              = REQ_TYPE_NVME_DMA;
            REQ_ENTRY(reqSlotTag)->reqOpt.dataBufFormat = REQ_OPT_DATA_BUF_ENTRY;
...
void IssueNvmeDmaReq(unsigned int reqSlotTag)
{
    ...
    {
        if(nvme_complete_flag == 1){
            pr_info("check auto_tx_dma done");
            ...
            pr_info("auto_tx_dma done");
            pr_info("reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag :%d",reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag);
            nvme_complete_flag = 0;
