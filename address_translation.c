...
void InitAddressMap()
{
    ...
    channelInfo        = (P_CH_INFO)CH_INFO_BUFFER_ADDR;
...
void InitBlockDieMap()
{
    ...
    // After the system erases, the channel information must be rewritten.
    if (eraseFlag == 0){
        recordChannelInfo();
    }
...
void recordChannelInfo(){
    uint32_t iWay = 0,iPBlk = 0,iPage = 3;
    pr_info("Record channel info start in each channel Way[%d] Blk[%d] Page[%d]",iWay,iPBlk,iPage);
    for ( uint32_t iCh = 0;iCh < USER_CHANNELS; iCh++){
        channelInfoBufferClear(iCh);
        channelInfo->ChInfo[iCh].ChNum = iCh;
        ...
        REQ_ENTRY(iReqEntry)->dataBufInfo.addr              = (uintptr_t)channelInfo->ChInfo[iCh].byte;
...
{
    pr_info("Check channel info start");
    uint32_t iWay = 0,iPBlk = 0,iPage = 3;
    for ( uint32_t iCh = 0;iCh < USER_CHANNELS ; iCh++){
        channelInfoBufferClear(iCh);
        uint32_t iReqEntry = GetFromFreeReqQ();
        ...
        if(channelInfo->ChInfo[iCh].ChNum != iCh){
            return true;
        }
    }
    return false;
...
{
    pr_info("Clear channel info buffer CH[%d] ADDR:0x%x",iCh,channelInfo->ChInfo[iCh].byte);
    memset(channelInfo->ChInfo[iCh].byte,0,BYTES_PER_SLICE);
}
