...
void monitor_check_bad_block(uint32_t iCh, uint32_t iWay, uint32_t iPBlk, uint32_t iPage){
    pr_info("monitor_check_bad_block");
    unsigned char *markPointer0;
    unsigned char *markPointer1;
    bool bad_block_flag = false;
    int BadPageNum = 0;
    int record[256];
    uint32_t iDie      = PCH2VDIE(iCh, iWay);

    for(uint16_t iPage = 0;iPage < ROWS_PER_SLC_BLOCK;iPage++){
        ...
        // print request info
        markPointer0 = (unsigned char *)((uintptr_t)MONITOR_DIE_DATA_BUF(iDie).byte + BAD_BLOCK_MARK_BYTE0);
        markPointer1 = (unsigned char *)((uintptr_t)MONITOR_DIE_DATA_BUF(iDie).byte + BAD_BLOCK_MARK_BYTE1);
        if(*markPointer0 == CLEAN_DATA_IN_BYTE && *markPointer1 == CLEAN_DATA_IN_BYTE){
            pr_debug("CH[%d] WAY[%d] BLK[%d] PAGE[%d] page is well",iCh,iWay,iPBlk,iPage);
        }
        else{
            bad_block_flag = TRUE;
            record[BadPageNum] = iPage;
            BadPageNum++;
            pr_info("CH[%d] WAY[%d] BLK[%d] PAGE[%d] page is bad",iCh,iWay,iPBlk,iPage);
        }
    ...
    if(bad_block_flag){
        pr_info("CH[%d] WAY[%d] BLK[%d] is bad block",iCh,iWay,iPBlk);
        for(int iPage = 0;iPage < BadPageNum;iPage++){
            pr_info("The read error occurred in PAGE[%d]",record[iPage]);
        }
    }
