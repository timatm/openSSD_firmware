...
static char filename[NMC_FILENAME_MAX_BYTES + 1] = {0}; // for modification in debugging mode

void handle_monitor_nmc(NVME_ADMIN_COMMAND *nvmeAdminCmd)
{
    uint32_t mode = nvmeAdminCmd->dword10; // dword10 is used to specify the monitor mode

    if (nvmeAdminCmd->OPC == ADMIN_MONITOR_NMC_MAPPING)
    {
        uint32_t iFile = nvmeAdminCmd->dword11;
        uint32_t fid   = nvmeAdminCmd->dword11;
        uint32_t iDie  = nvmeAdminCmd->dword11;
        uint32_t vba   = nvmeAdminCmd->dword12;
        ...
        {
        case 1:
            nmcRecordBlock(iDie, vba);
            break;
        case 2:
            nmcRecoverMappingDir();
            break;

        case 3: /* specify file by index */
            if (fid == 0)
                strncpy(filename, "img2", NMC_FILENAME_MAX_BYTES);
            else if (fid == 1)
                strncpy(filename, "sample1.tiff", NMC_FILENAME_MAX_BYTES);
            else if (fid == 2)
                strncpy(filename, "unet.model", NMC_FILENAME_MAX_BYTES);
            else
                ; // modify the global string before searching

            info = nmcSearchFile(filename);

            if (info.type == NMC_FILE_TYPE_NONE)
                pr_warn("File \"%s\" not found or has unsupported type...", filename);
            else
            {
                pr_debug("Mapping Table Location of File \"%s\" (type: %u)", filename, info.type);
                for (int iCh = 0; iCh < USER_CHANNELS; ++iCh)
                {
                    loc = info.mapping[iCh];
                    pr_debug("\t Ch[%u]: W[%u].D[%u].P[%u]", iCh, loc.iWay, loc.iDir, loc.iPage);
                }
            }
            break;

        case 4:
            /* search by location table index */
            info = nmcSearchFileIdx(iFile);

            if (info.type != NMC_FILE_TYPE_NONE)
            {
                for (uint8_t iCh = 0; iCh < USER_CHANNELS; ++iCh)
                {
                    addr = nmcReadMappingTable(iCh, info.mapping[iCh]);
                    nmcDumpMappingTable((void *)addr, true);
                }
            }
            break;

        default:
            pr_error("Unsupported NMC monitor command: %u", nvmeAdminCmd->OPC);
            break;
        }
    }
    else
...
void handle_nvme_admin_cmd(NVME_COMMAND *nvmeCmd)
{
    ...
    case ADMIN_MONITOR_NMC_MAPPING:
    {
        handle_monitor_nmc(nvmeAdminCmd);
